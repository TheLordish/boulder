...
(function (App) {
  ...
  var gym = {
    ...
    removeSession: function (date, id) {
      var list = this.sessions(date).filter(function (s) { return s.id !== id; });
      this.saveSessions(date, list);
      if (this.open === id) this.open = null;
      this.render();
    },

    /* ---- the clock ----
       Elapsed time for the session, and a rest timer that restarts on every set
       so you can see how long you have actually been sitting there. */
    elapsed: function () {
      var s = this.current();
      if (!s) return 0;
      return Math.max(0, Math.floor((Date.now() - new Date(s.at).getTime()) / 1000));
    },
    restSince: function () {
      return this._rest ? Math.floor((Date.now() - this._rest) / 1000) : null;
    },
    markRest: function () { this._rest = Date.now(); },
    clearRest: function () { this._rest = null; },
    clock: function (sec) {
      if (sec == null) return "";
