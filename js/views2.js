...
(function (App) {
  ...
  /* ─────────────── body composition ─────────────── */
  /* Segment values drawn on a body rather than listed in a table, because the
     thing you want to see is which side is behind, and a table hides that. */
  function bodyMap(last, prev) {
    var seg = App.comp.SEG;
    var mu = last.segMuscle || {}, fa = last.segFat || {};
    if (!Object.keys(mu).length && !Object.keys(fa).length) return null;
    var pm = (prev && prev.segMuscle) || {};

    var vals = seg.map(function (s) { return mu[s[1]]; }).filter(function (v) { return v != null; });
    var hi = vals.length ? Math.max.apply(null, vals) : 1;

    function part(key, x, y, w, h, rx) {
      var v = mu[key], p = pm[key];
      var fill = v == null ? 0 : Math.max(0.14, v / hi);
      var d = (v != null && p != null) ? v - p : null;
      var col = d === null ? "" : d > 0.05 ? "up" : d < -0.05 ? "down" : "";
      return '<g class="bp ' + col + '">' +
        '<rect x="' + x + '" y="' + y + '" width="' + w + '" height="' + h + '" rx="' + rx +
        '" fill="var(--ink)" opacity="' + fill.toFixed(2) + '"/>' +
        '</g>';
    }
    function label(key, x, y, anchor) {
      var v = mu[key], f = fa[key], p = pm[key];
      if (v == null && f == null) return "";
      var d = (v != null && p != null) ? v - p : null;
      return '<text x="' + x + '" y="' + y + '" text-anchor="' + anchor + '" font-size="7" fill="var(--ink)">' +
        (v != null ? v.toFixed(1) : "\u2014") + '</text>' +
        (d !== null ? '<text x="' + x + '" y="' + (y + 8) + '" text-anchor="' + anchor +
          '" font-size="6" fill="' + (d > 0.05 ? "var(--good)" : d < -0.05 ? "var(--over)" : "var(--dim)") + '">' +
          (d > 0 ? "+" : "") + d.toFixed(1) + '</text>' : "") +
        (f != null ? '<text x="' + x + '" y="' + (y + (d !== null ? 17 : 9)) + '" text-anchor="' + anchor +
          '" font-size="6" fill="var(--dim)">' + f.toFixed(1) + ' fat</text>' : "");
    }

    var box = node("div", "bodymap");
    box.innerHTML =
      '<svg viewBox="0 0 200 150" role="img" aria-label="Segmental muscle mass">' +
      '<circle cx="100" cy="20" r="10" fill="var(--line)"/>' +
      part("tr", 86, 33, 28, 48, 7) +
      part("la", 66, 36, 13, 40, 6) + part("ra", 121, 36, 13, 40, 6) +
      part("ll", 88, 85, 11, 52, 5) + part("rl", 101, 85, 11, 52, 5) +
      label("la", 60, 60, "end") + label("ra", 140, 60, "start") +
      label("tr", 100, 60, "middle") +
      label("ll", 60, 118, "end") + label("rl", 140, 118, "start") +
      '<text x="100" y="146" text-anchor="middle" font-size="6" fill="var(--dim)">' +
      'muscle in kg, shade shows relative size, change since the previous scan below</text>' +
      '</svg>';
    return box;
  }
  ...
  App.renderComp = function () {
    ...
    if (!dates.length) {
      box.appendChild(node("div", "empty",
        "No scans yet. Add one by hand, photograph the printout, or upload the PDF the gym sent."));
      el("compBig").textContent = "\u2014";
      el("compSub").textContent = "nothing recorded";
      return;
    }

    var prev = dates.length > 1 ? c.data[dates[dates.length - 2]] : null;
    ...
    el("compSub").textContent = "% body fat on " + short(dates[dates.length - 1]) +
      ...
      (dates.length < 3 ? " \u00b7 too few to read a trend" : "");

    /* The three numbers that decide whether a phase is working, with their
       change since the previous scan and across the whole record. */
    var head = node("div", "cstats");
    [["skm", "Muscle", "kg"], ["fatMass", "Fat", "kg"], ["weight", "Weight", "kg"]].forEach(function (k) {
      if (last[k[0]] == null) return;
      var d = c.delta(k[0]);
      var first = c.data[dates[0]][k[0]];
      var all = first != null ? Math.round((last[k[0]] - first) * 10) / 10 : null;
      var good = k[0] === "skm" ? 1 : -1;          // more muscle good, more fat not
      var cell = node("div", "cstat");
      cell.innerHTML = '<div class="cv">' + last[k[0]].toFixed(1) + '<small>' + k[2] + '</small></div>' +
        '<div class="ck">' + k[1] + '</div>' +
        (d !== null ? '<div class="cd ' + (d * good > 0 ? "up" : d * good < 0 ? "down" : "") + '">' +
          (d > 0 ? "+" : "") + d + ' last scan</div>' : "") +
        (all !== null && dates.length > 1 ? '<div class="cd sub">' + (all > 0 ? "+" : "") + all + ' overall</div>' : "");
      head.appendChild(cell);
    });
    box.appendChild(head);

    var map = bodyMap(last, prev);
    ...
    box.appendChild(node("div", "clabel", "Everything measured, and the change since last time"));
    box.appendChild(tbl);

    var hist = node("div", "kv");
    dates.slice().reverse().slice(0, 8).forEach(function (dt) {
      var row = node("div", "kvrow");
      row.appendChild(btn("k link", short(dt), function () { App.comp.editing = dt; App.renderComp(); }));
      row.appendChild(node("span", "v", c.data[dt].weight != null ? c.data[dt].weight + " kg" : ""));
      row.appendChild(node("span", "d", c.data[dt].fatPct != null ? c.data[dt].fatPct + "%" : ""));
  ...
  function gymSession(box) {
      ...
      if (dates.length) {
        ...
        dates.forEach(function (date) {
          g.sessions(date).forEach(function (ses) {
            ...
            r.appendChild(iconBtn("cpx", "trash", function () {
              if (window.confirm("Delete this session?")) g.removeSession(date, ses.id);
            }, "Delete session on " + short(date)));
            list.appendChild(r);
          });
        });
    ...
    var clock = node("div", "clock");
    var el1 = node("span", "ct"), el2 = node("span", "rt");
    clock.appendChild(el1); clock.appendChild(el2);
    var stop = btn("mini no", "End", function () { g.endSession(); });
    clock.appendChild(stop);
    box.appendChild(clock);
    function tick() {
      if (!document.body.contains(el1)) { clearInterval(App._gymTimer); return; }
      el1.textContent = g.clock(g.elapsed()) + " elapsed";
      var rest = g.restSince();
      el2.textContent = rest === null ? "" : g.clock(rest) + " rest";
      el2.className = "rt" + (rest !== null && rest > 240 ? " long" : "");
    }
    clearInterval(App._gymTimer);
    App._gymTimer = setInterval(tick, 1000);
    tick();
    ...
    end.appendChild(btn("ghost danger", "Discard", function () {
      if (window.confirm("Throw this session away?")) {
        g.removeSession(today(), s.id); g.open = null; g.render();
      }
    }));
    box.appendChild(end);
  }
