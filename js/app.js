(function (App) {
  ...
  /* The day divided the way people actually eat it. An entry lands in the last
     slot whose time has passed, so a 15:00 snack sits under Lunch rather than
     floating loose. Names and times are editable. */
  var DEFAULT_SLOTS = [
    { id: "b", name: "Breakfast", at: "05:00" },
    { id: "l", name: "Lunch", at: "12:00" },
    { id: "d", name: "Dinner", at: "18:00" },
    { id: "n", name: "Late", at: "22:00" }
  ];
  function slots() {
    var sl = state.targets.slots;
    return (Array.isArray(sl) && sl.length) ? sl : DEFAULT_SLOTS;
  }
  function slotFor(time) {
    var list = slots(), found = list[0].id;
    if (!time || time === "--:--") return found;
    list.forEach(function (sl) { if (time >= sl.at) found = sl.id; });
    return found;
  }
  function groupBySlot(entries) {
    var by = {};
    slots().forEach(function (sl) { by[sl.id] = []; });
    entries.forEach(function (e) {
      var id = slotFor(e.time);
      (by[id] = by[id] || []).push(e);
    });
    return by;
  }
  ...
  function phase() { return state.targets.phase || "cut"; }
  function phaseLabel() {
    var p = phase();
    return p === "bulk" ? "Bulk" : p === "maintain" ? "Maintenance" : "Cut";
  }

  /* Calories judged against the phase. On a cut the ceiling is what matters, on
     a bulk the floor is, and on maintenance both edges do. Same buffer either
     way, so landing near the number always counts as hitting it. */
  function calorieZone(total, target, buffer) {
    if (!target) return "idle";
    buffer = buffer || 0;
    var p = phase();
    if (p === "bulk") {
      if (total >= target - buffer) {
        return total > target + buffer * 4 ? "near" : "ok";   // a big overshoot is still worth flagging
      }
      return total >= target * 0.8 ? "near" : "idle";
    }
    if (p === "maintain") {
      if (Math.abs(total - target) <= buffer) return "ok";
      if (Math.abs(total - target) <= buffer * 3) return "near";
      return total > target ? "over" : "idle";
    }
    if (total > target + buffer) return "over";
    if (total >= target - buffer) return "ok";
    if (total >= target * 0.8) return "near";
    return "idle";
  }
  ...
  var state = {
    ...
    targets: { kcal: 2000, protein: 150, lookup: false, weight: 0, rate: 0.5, buffer: 100, phase: "cut" },
    openId: null, busy: false, shown: 0, sugg: null, recipe: null, recipeBusy: false, lastRecipe: null, fit: null, fitBusy: false, fitText: "", mealSize: "meal", chartZoom: "in", clean: {}, stOpen: false, thread: [], askBusy: false, carry: {}, model: null
  };
  ...
  // ---- the coach ----
  var COACH_RULES =
    "You are reading someone's own training and body data and giving them an honest read. " +
    "They are in a PHASE_NAME phase, so judge everything against that intent.\n\n" +
    "THE QUESTION THAT MATTERS. Weight alone cannot tell them anything. They have four streams " +
    "that almost nobody has together: what they ate, what they weigh, what they lifted, and what " +
    "their body composition scans say. Use them together. When the scale moves, say what moved. " +
    "Weight down with skeletal muscle holding and lifts climbing is a clean cut. Weight down with " +
    "muscle falling and lifts dropping is too aggressive, and the answer is more protein and a " +
    "smaller deficit. Weight up with lean mass leading is a good bulk; weight up with fat leading " +
    "is not. Say which of these is happening and what it implies.\n\n" +
    "READ THE SCANS CAREFULLY. Bioimpedance is noisy. Hydration alone moves body fat percentage by " +
    "a point or two, so two scans a month apart can differ more from water than from anything " +
    "real. Judge the direction across four or five scans. Never call a single pair of scans proof " +
    "of muscle gained or lost. If there are fewer than three scans, say the composition picture is " +
    "not readable yet.\n\n" +
    "TRAINING. Strength holding or climbing during a deficit is the best available evidence that " +
    "muscle is being kept. If estimated one-rep maxes are falling while weight falls, treat that " +
    "as the warning it is. Mention a specific lift and its numbers rather than talking generally. " +
    "If a muscle group is behind in the segmental data, connect it to whether the plan trains it.\n\n" +
    "MAINTENANCE. If a measured maintenance figure is given, it came from their own intake against " +
    "their own weight change, so trust it over any formula. If logging adherence is under about 70 " +
    "percent, say the numbers cannot carry a conclusion yet and that logging consistently is the " +
    "first fix.\n\n" +
    "STYLE. Specific and quantitative, using their actual figures. Direct, no cheerleading, no " +
    "moralising about food, never shame them. Say when the data will not support a conclusion " +
    "instead of inventing a trend. If their rate exceeds about one percent of bodyweight a week on " +
    "a cut, say plainly that it is faster than is useful. If protein is well under target during a " +
    "deficit, flag it, that is what protects muscle.\n\n" +
    ...
    '"trend":"2 to 4 sentences saying what actually moved, fat or muscle, and why",' +
    '"working":["1 to 3 short points the data supports"],' +
    '"watch":["1 to 3 short points that are slipping"],' +
  ...
  function coachPayload() {
    ...
    /* The other two streams. Without these the coach is just reading a scale. */
    lines.push("");
    lines.push("== TRAINING LOG ==");
    lines.push(App.gym ? App.gym.summary() : "No training logged.");
    lines.push("");
    lines.push("== BODY COMPOSITION SCANS ==");
    lines.push(App.comp ? App.comp.summary() : "No scans recorded.");
    if (App.comp) {
      var n = App.comp.dates().length;
      lines.push("Scan count " + n + ". " + (n < 3
        ? "Fewer than three, so the composition picture is not readable yet."
        : "Judge direction across all of them, not the last pair."));
    }
    return lines.join("\n");
  }
  ...
  async function askCoach() {
    ...
    try {
      var data = await App.api.call([{ role: "user",
        content: COACH_RULES.replace("PHASE_NAME", phaseLabel().toLowerCase()) +
    ...
    /* The reply may land after you have moved to another page. Redraw only the
       panel that owns it, so the result is waiting when you come back rather
       than quietly lost. */
    if (state.view === "ask" && state.askPane === "read") render();
    else renderCoach();
  }
  ...
  function assistantContext() {
    ...
    L.push("They are in a " + phaseLabel().toLowerCase() + " phase.");
  ...
  function renderAdvice(totals) {
    ...
    if (phase() === "bulk" && left > (state.targets.buffer || 0)) {
      var card0 = document.createElement("div");
      card0.className = "card sugg";
      var h0 = document.createElement("h3");
      h0.textContent = left + " kcal short of the surplus";
      var p0 = document.createElement("p");
      p0.textContent = "On a bulk this is the gap that matters. Adding it in one dense meal is " +
        "easier than three light ones.";
      card0.appendChild(h0); card0.appendChild(p0);
  ...
  function renderWeightBlock() {
    ...
    var togo = cur - tgt;
    var gaining = span < 0;                      // goal above start, so a bulk
    var reached = gaining ? togo >= -0.05 : togo <= 0.05;
    ...
    box.innerHTML =
      '<div class="wgtop">' +
        ...
        '<span class="wgtogo">' + (reached
          ? '<b>goal reached</b>'
          : '<b>' + Math.abs(togo).toFixed(1) + ' kg</b> to ' + (gaining ? "gain" : "go")) +
      ...
      '<div class="wgends">' +
        ...
        '<span class="wgpct">' + Math.round(pct) + '% of the way' +
          (Math.abs(doneKg) > 0.05 ? ', ' + Math.abs(doneKg).toFixed(1) +
          (doneKg > 0 ? ' kg down' : ' kg on') : '') + '</span>' +
  ...
  /* One meal, collapsed to a line. Everything else opens on tap. */
  function mealRow(e) {
  ...
  function renderLog() {
    ...
    var buf = state.targets.buffer || 0;
    var zone = calorieZone(t.kcal, eff, buf);
    over = t.kcal > eff + buf;
    countTo(el("remaining"), left, "big " + zone);
    var ph = phase();
    el("heroSub").textContent = (
      zone === "ok" ? "on target, " + eff.toLocaleString() + " kcal"
      : over ? "over " + eff.toLocaleString() + " kcal"
      : ph === "bulk" ? "still to eat, of " + eff.toLocaleString()
    ...
    if (!state.entries.length) {
      ...
      em.textContent = isToday(state.date) ? "Nothing logged yet." : "Nothing logged this day.";
      rows.appendChild(em);
    } else {
      var by = groupBySlot(state.entries);
      state.openSlots = state.openSlots || {};
      slots().forEach(function (sl) {
        var list = by[sl.id] || [];
        if (!list.length) return;
        var st = list.reduce(function (a, e) {
          a.k += sum(e.items, "kcal"); a.p += sum(e.items, "p"); return a;
        }, { k: 0, p: 0 });
        var open = state.openSlots[sl.id] !== false;

        var head = document.createElement("button");
        head.className = "slot" + (open ? " open" : "");
        var chev = document.createElement("span"); chev.className = "schev";
        chev.innerHTML = App.icon(open ? "down" : "right", 13);
        var nm2 = document.createElement("span"); nm2.className = "sname"; nm2.textContent = sl.name;
        var cn = document.createElement("span"); cn.className = "scount"; cn.textContent = list.length;
        var kc2 = document.createElement("span"); kc2.className = "skc"; kc2.textContent = st.k;
        var pr2 = document.createElement("span"); pr2.className = "sp"; pr2.textContent = st.p + "p";
        head.appendChild(chev); head.appendChild(nm2); head.appendChild(cn);
        head.appendChild(kc2); head.appendChild(pr2);
        head.onclick = function () {
          state.openSlots[sl.id] = !open;
          save("openSlots", state.openSlots);
          render();
        };
        rows.appendChild(head);
        if (open) list.forEach(function (e) { rows.appendChild(mealRow(e)); });
      });
    }
  ...
  function renderMonth() {
    ...
    /* Averages across the month, from the full day records rather than the
       calorie rollup, so protein and the rest are real numbers not estimates. */
    var macro = { k: 0, p: 0, c: 0, f: 0, n: 0 };
    var ym = state.month.getFullYear() + "-" + pad(state.month.getMonth() + 1);
    App.store.keys("day:").forEach(function (key) {
      if (key.indexOf("day:" + ym) !== 0) return;
      var t = dayTotals(load(key, []).map(normalise));
      if (!t.kcal) return;
      macro.k += t.kcal; macro.p += t.p; macro.c += t.c; macro.f += t.f; macro.n++;
    });
    ...
    var under = logged.filter(function (v) {
      return calorieZone(v, state.targets.kcal, state.targets.buffer || 0) === "ok";
    }).length;
    var rows2 = [[logged.length ? avg.toLocaleString() : "\u2014", "kcal a day"],
                 [logged.length ? under + "/" + logged.length : "\u2014", "days on target"]];
    if (macro.n) {
      rows2.push([Math.round(macro.p / macro.n) + "g", "protein a day"]);
      rows2.push([Math.round(macro.c / macro.n) + "g", "carbs"]);
      rows2.push([Math.round(macro.f / macro.n) + "g", "fat"]);
    }
    rows2.forEach(function (s) {
  ...
  function renderHistogram() {
    ...
    vals.forEach(function (x) {
      ...
      b.className = "hbar " + (!x.v ? "none" : calorieZone(x.v, tgt, state.targets.buffer || 0));
  ...
  function renderChart(series) {
    ...
    /* Two framings. Zoomed in shows the weigh-ins at a readable scale, which is
       what you look at daily. Zoomed out stretches to the furthest checkpoint so
       the whole plan is visible at once, at the cost of squashing recent data. */
    var logged = tLast - t0;
    var t1;
    if (state.chartZoom === "out") {
      t1 = tLast;
      state.checkpoints.forEach(function (c) {
        var ct = parseISO(c.date).getTime();
        if (ct > t1) t1 = ct;
      });
      if (state.targets.weight && t1 === tLast) t1 = tLast + 180 * DAY;
    } else {
  ...
  function renderCheckpoints() {
    ...
    if (!state.checkpoints.length) {
      var e = document.createElement("div"); e.className = "cpempty";
      e.textContent = "No checkpoints. Try 85 kg by 15 December.";
      list.appendChild(e); return;
    }
    /* One chip each rather than a row each, so a handful fit without scrolling
       and the verdict is still visible on every one. */
    state.checkpoints.forEach(function (c) {
      var st = cpStatus(c);
      var chip = document.createElement("span");
      chip.className = "cpchip " + st.key;
      var kg = document.createElement("b"); kg.textContent = c.kg;
      var wh = document.createElement("span"); wh.className = "cpwhen";
      wh.textContent = parseISO(c.date).toLocaleDateString(undefined, { day: "numeric", month: "short" });
      chip.appendChild(kg); chip.appendChild(wh);
      chip.title = st.text;
      var x = document.createElement("button"); x.className = "cpx";
      x.innerHTML = App.icon("x", 11);
      x.setAttribute("aria-label", "Remove checkpoint at " + c.kg + " kg");
  ...
  function renderWeight() {
    ...
    var zb = el("zoomBtn");
    if (zb) {
      var out = state.chartZoom === "out";
      zb.setAttribute("aria-pressed", String(out));
      zb.textContent = out ? "Zoom in" : "Show goals";
    }
    renderRate();
    renderCoach();
    renderCheckpoints();
  ...
  /* One place builds the payload, so the button and the automatic backup can
     never drift apart. */
  /* Anything that writes to the store from outside the app loop must call this,
     or the in-memory copy silently diverges from disk. */
  App.reloadFromStore = function () {
    state.weights = load("weights", {});
    state.totals = load("totals", {});
    state.clean = load("clean", {});
    state.favs = load("favorites", []);
    state.checkpoints = load("checkpoints", []);
  ...
  App.exportAll = async function () {
    ...
    return JSON.stringify({
      app: "boulder", v: 3, made: new Date().toISOString(),
      settings: state.targets, favorites: state.favs, weights: state.weights,
      totals: state.totals, pantry: state.pantry, checkpoints: state.checkpoints,
      clean: state.clean, carry: state.carry,
      gymPlan: App.store.get("gym:plan", null), comp: App.store.get("comp", null),
      meals: App.store.get("meals", null), reminders: App.store.get("reminders", null),
      activity: App.store.get("activity", null),
  ...
  App.fillTargets = function () {
    var ps = el("phaseSel");
    if (ps) ps.querySelectorAll("button").forEach(function (b) {
      b.setAttribute("aria-selected", String(b.getAttribute("data-phase") === phase()));
    });
  ...
  App.setPhase = function (p) {
    state.targets.phase = p;
    save("settings", state.targets);
    App.fillTargets();
    render();
  };
  App.phase = phase;
  App.phaseLabel = phaseLabel;
  App.toggleZoom = function () {
    state.chartZoom = state.chartZoom === "out" ? "in" : "out";
    render();
  };
  ...
  /* A whole report rather than a snapshot: PDF, Word or an image. Nothing is
     saved until you have seen the numbers, because a misread scan would poison
     the trend for months. */
  App.readScanDoc = async function (file) {
    if (!App.docs) { App.compStatus("Document reading is unavailable.", true); return; }
    App.compStatus("Reading " + (file.name || "the file"));
    try {
      var content = await App.docs.toContent(file, App.comp.READ_RULES);
      var data = await App.api.call([{ role: "user", content: content }], { tier: "fast" });
      var o = App.api.json(data);
      var date = /^\d{4}-\d{2}-\d{2}$/.test(o.date || "") ? o.date : today();
      if (App.comp.put(date, o)) {
        App.compStatus("Read " + (file.name || "the report") + ". Check it against the sheet before it counts.");
        App.comp.editing = date;
        App.renderComp();
      } else App.compStatus("Nothing readable in that file.", true);
    } catch (e) {
      App.compStatus(e && e.kind === "nokey" ? "Add your API key first."
        : (e && e.message) || "Could not read that file.", true);
    }
  };
  ...
  App.boot = function () {
    setStatus("Loading");
    var r = [
      load("settings", { kcal: 2000, protein: 150, lookup: false, weight: 0, rate: 0.5, buffer: 100, phase: "cut" }),
    ...
    state.targets = Object.assign({ kcal: 2000, protein: 150, lookup: false, weight: 0, rate: 0.5, buffer: 100, phase: "cut" }, r[0]);
    if (!state.targets.phase) state.targets.phase = "cut";
    ...
    state.clean = r[8] || {};
    state.openSlots = load("openSlots", {});
    state.carry = r[9] || {};
