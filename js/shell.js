...
(function (App) {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (el("compDoc")) el("compDoc").onclick = function () { el("compFile").click(); };
    if (el("compFile")) el("compFile").onchange = function (e) {
      var f = e.target.files && e.target.files[0];
      e.target.value = "";
      if (f) App.readScanDoc(f);
    };
    if (el("zoomBtn")) el("zoomBtn").onclick = function () {
      App.toggleZoom();
    };
    ...
    if (ps) ps.querySelectorAll("button").forEach(function (b) {
      b.onclick = function () { App.setPhase(b.getAttribute("data-phase")); };
    });
    ...
    /* the native panels only mean anything inside the packaged app */
    if (App.native && App.native.is()) {
      var hf = el("healthFold"), bf = el("backupFold");
      if (hf) hf.hidden = false;
      if (bf) bf.hidden = false;
      var hs = function (m, bad) {
        var n = el("hcStat"); if (!n) return;
        n.className = "note" + (bad ? " err" : ""); n.textContent = m;
      };
      App.native.healthAvailable().then(function (r) {
        hs(r.available ? "Health Connect found. Press Connect to choose what Boulder may read."
                       : r.reason);
      });
      if (el("hcConnect")) el("hcConnect").onclick = async function () {
        hs("Asking for permission");
        var g = await App.native.askHealth();
        if (!g) { hs("Could not reach Health Connect.", true); return; }
        var ok = Object.keys(g).filter(function (k) { return g[k]; });
        hs(ok.length ? "Allowed: " + ok.join(", ").toLowerCase().replace(/read_/g, "")
                     : "Nothing was allowed. Open Health Connect settings to change it.", !ok.length);
      };
      if (el("hcSync")) el("hcSync").onclick = async function () {
        hs("Reading the last 30 days");
        var r = await App.native.syncHealth(30);
        if (!r) { hs("Nothing to read.", true); return; }
        hs("Brought in " + r.weights + " weigh-ins, " + r.fat + " body fat, " + r.lean +
           " lean mass, " + r.steps + " days of steps, " + r.workouts + " workouts." +
           (r.errors.length ? " Not permitted: " + r.errors.join(", ") + "." : ""));
        if (App.refresh) App.refresh();
      };
      if (el("hcOpen")) el("hcOpen").onclick = function () { App.native.openHealthSettings(); };
      if (el("bkNow")) el("bkNow").onclick = async function () {
        var n = el("bkStat");
        n.className = "note"; n.textContent = "Writing";
        var p = await App.native.backup(true);
        n.textContent = p ? "Saved to Documents/" + p : "Could not write the backup.";
        if (!p) n.className = "note err";
      };
      var lastB = App.store.get("lastBackup", "");
      if (el("bkStat")) el("bkStat").textContent = lastB ? "Last backup " + lastB : "No backup yet.";
    }
